...
if (process.env.KEY_VAULT_URL) {
    ...
    secretClient = new keyvault_secrets_1.SecretClient(process.env.KEY_VAULT_URL, credential);
}
/**
...
async function getSecret(secretName, envVarName) {
    ...
    if (!envValue) {
        throw new Error(`Secret '${secretName}' not found in Key Vault and environment variable '${envVarName}' is not set`);
    }
    return envValue;
}
/**
